...
/**
 * Parse the data returned from the stops list remote API, because it's in JSONP format, not JSON
 * @param {string} input
 * @returns {array} output
 */
const parseStopsListData = input => {
    try {
        const res = JSON.parse(input.replace(/^\(/, '').replace(/\)[;]*$/, ''));
        const { result } = res;
...
const fetchStopsListData = () => {
    return new Promise((resolve, reject) => {
        ...
        request(options, (err, response) => {
            ...
            if (statusCode === 200) {
                resolve(parseStopsListData(body));
                return;
            }
