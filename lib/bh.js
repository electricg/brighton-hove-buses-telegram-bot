  ...
  return new Promise((resolve, reject) => {
    ...
    request(options, (err, response) => {
      ...
      if (response.statusCode === 200) {
        const res = JSON.parse(response.body.replace(/^\(/, '').replace(/\);$/, ''));
 ...
 */
const getNearbyStops = (here) => {
  return getStopsListData()
    .then((res) => {
      const output = getNearbyLocations(here, res);
