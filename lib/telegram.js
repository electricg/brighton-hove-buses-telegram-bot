...
const createResponse = (messageId, stop, service) => {
  return bh
    ...
    .then(res => {
      let message =
        `${res.stopName} (${res.bearing})` + ` @ ${res.lastUpdate}\n`;

      res.times.forEach(item => {
        message += `${item.service} to ${item.destination} - ${
          item.timeLabel
        }\n`;
      });
...
const sendResponse = (bot, msg, match) => {
  ...
  createResponse(messageId, stop, service).then(res => {
...
const sendLocation = (bot, msg, range) => {
  ...
  createResponseLocation(messageId, msg.location, range).then(res => {
    bot.sendMessage(chatId, res.message, res.opts);
  });
};
