...
const _createResponseBusstop = _telegram.__get__('createResponseBusstop');
const _createResponseLocation = _telegram.__get__('createResponseLocation');
const _sendResponseBusstop = _telegram.__get__('sendResponseBusstop');
...
describe('telegram', () => {
  ...
  describe('createResponseBusstop', () => {
    it('should succeed with only stopcode', (done) => {
      const messageId = 'abc';
      _createResponseBusstop(messageId, stopcode)
        .then((res) => {
          ...
          console.log(res);
          const regex = new RegExp('^briapaw - Seven Dials \\(E\\) @ \\d{1,2}:\\d{2}\\n([a-zA-Z0-9]{1,3} to [-\\w: ]+\\n)*$');
          regex.test(res.message).should.equal(true);
    ...
    //   const messageId = 'abc';
    //   _createResponseBusstop(messageId, stopcode, servicename)
    //     .then((res) => {
    //       const expectedOutput = {
    //         opts: {
    ...
    //   _createResponseBusstop(messageId, wrongStopcode)
    //     .then((res) => {
    //       const expectedOutput = {
    ...
    //   _createResponseBusstop(messageId, errorStopid)
    //     .then((res) => {
    //       const expectedOutput = {
  ...
  //     _sendResponseBusstop(bot, msg, match);
